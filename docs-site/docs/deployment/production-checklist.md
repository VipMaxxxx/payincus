...
- `PAYMENT_CALLBACK_IP_WHITELIST is empty`：表示没有配置统一支付回调 IP 白名单。若支付商有固定回调出口，建议在 `.env` 配置 `PAYMENT_CALLBACK_IP_WHITELIST`；若支付商没有固定出口，且运营方明确不要求 IP 白名单，可以设置 `PAYMENT_CALLBACK_IP_WHITELIST_REQUIRED=false`。该策略必须保持 `PAYMENT_CALLBACK_SKIP_IP_WHITELIST=false`，并确认该渠道回调仍会经过签名验签、交易状态、金额匹配和 `payment_callbacks` 幂等校验后才会入账。DB readiness 会额外指出哪些 active 支付渠道没有内置默认回调 IP；设置显式豁免后，该项会记录为已接受策略，不再作为未处理告警。
