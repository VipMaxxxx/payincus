...
- `PAYMENT_CALLBACK_IP_WHITELIST is empty`: no global payment callback IP allowlist is configured. If the payment provider publishes fixed callback source IPs, configure `PAYMENT_CALLBACK_IP_WHITELIST` in `.env`. If the provider does not provide stable source IPs and the operator explicitly does not require an IP allowlist, set `PAYMENT_CALLBACK_IP_WHITELIST_REQUIRED=false`. Keep `PAYMENT_CALLBACK_SKIP_IP_WHITELIST=false`, and confirm that callbacks still pass signature verification, trade-status validation, amount matching and `payment_callbacks` idempotency before balance credit. DB readiness identifies active payment providers without built-in default callback IPs; with the explicit waiver set, this is recorded as an accepted policy instead of an unresolved warning.
