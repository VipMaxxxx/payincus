...
PayIncus API 分为同源内部 API、Public API、Agent API、OAuth Provider 和 WebSocket。第三方开发者和自动化系统应优先使用稳定的 `/api/v1` Public API；管理后台和用户端内部接口不作为第三方稳定契约。
...
| Token | 来源 | 用途 |
| --- | --- | --- |
| `pat_` | 用户在 PayIncus 创建的 API Token | 服务端集成和自动化脚本 |
...
Public API 不开放直接支付建单、支付回调、退款、直接余额写入、服务创建/删除/迁移、敏感资料修改和后台管理。需要这些能力时，应通过 PayIncus 内部状态机或后台审核完成。
