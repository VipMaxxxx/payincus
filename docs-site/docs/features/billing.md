...
| 订单中心 | 在 `/admin/orders` 统一查看充值订单和实例账单，支持按用户、类型和状态筛选、查看详情、处理充值异常和发起人工调账。 |
...
- `orders`：用户端和后台的统一订单中心，聚合充值订单与实例计费记录；后台页面复用充值人工完成、标记失败和余额调账接口处理异常订单。
- `balance`：余额变动和账务流水。
...
- 支付渠道密钥只能后端保存和使用。
- 管理员手动调账必须写审计日志。
- 订单中心的人工完成、失败标记和调账必须填写可追溯原因，不能绕过已有充值入账和余额流水逻辑。
- 充值成功但通知失败时，账务状态仍应准确。
...
- 后台订单详情可以对待支付或处理中充值订单执行手动完成/标记失败，并可对关联用户执行带原因的人工调账。
