...
官方市场会提供 `AI 工单助手` 插件包。管理员需要先在生产环境配置 `PLUGIN_MARKET_INDEX_URL`，然后进入后台插件中心的“插件市场”页刷新并安装。安装完成后，在“已安装”页启用插件，并在插件详情的“插件设置页面”查看状态；配置 JSON 可套用默认模板后再填写 OpenAI 兼容模型地址、模型名、API key、接管模式、自动分类、置信度阈值和限流参数。
