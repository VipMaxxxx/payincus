...
assert.ok(adminApi.includes("http.post(`/balance/admin/${userId}/adjust`, { amount, remark })"), 'admin API wrapper must reuse audited balance-adjust route')
...
assert.ok(adminOrderView.includes('api.admin.completeRechargeOrder') && adminOrderView.includes('api.admin.failRechargeOrder') && adminOrderView.includes('api.admin.adjustUserBalance'), 'order actions must reuse existing audited APIs')
