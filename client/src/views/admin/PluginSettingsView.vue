...
import type { PluginConfigValue, PluginPageManifest, PluginRecord, TicketAiStatusResponse } from '@/types/api'
...
async function loadPage(): Promise<void> {
  ...
  try {
    ...
    toast.error('加载插件设置失败：' + (err?.message || String(err)))
    plugin.value = null
    configs.value = []
  } finally {
    loading.value = false
  ...
  <div class="p-6 space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-themed-muted">独立插件设置页。插件安装、启用和卸载仍在插件中心维护。</p>
      </div>
    ...
    <template v-else-if="plugin">
      <section class="grid gap-4 md:grid-cols-4">
        ...
        <div class="rounded-lg border border-themed bg-themed-surface p-4">
          ...
          <div class="mt-2 text-xl font-semibold text-themed">{{ plugin.sourceType === 'market' ? '插件市场' : '上传安装' }}</div>
        </div>
        <div class="rounded-lg border border-themed bg-themed-surface p-4">
      ...
      <section v-if="isAiTicketPlugin" class="space-y-6">
        ...
        <div v-if="!plugin.enabled" class="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
          插件当前未启用。请先在插件中心启用插件，然后再保存和使用 AI 工单助手配置。
        </div>
      ...
      <section v-else class="rounded-lg border border-themed bg-themed-surface p-5">
        <h2 class="text-lg font-semibold text-themed">插件设置页面</h2>
        <p class="mt-1 text-sm text-themed-muted">该插件的设置页面由插件包提供。</p>
    ...
    <div v-else class="rounded-lg border border-themed bg-themed-surface p-10 text-center">
      ...
      <p class="mt-2 text-sm text-themed-muted">请返回插件中心确认插件是否已安装。</p>
      <button class="btn-primary mt-4" @click="router.push('/admin/plugins')">返回插件中心</button>
    </div>
  </div>
</template>
